...
exports.submitLead = async (req, res) => {
  ...
  try {
    await ensureTable()
    await db.execute(
      'INSERT INTO ms_biolink_leads (site_id, name, phone, email, message) VALUES (?, ?, ?, ?, ?)',
...
exports.listLeads = async (req, res) => {
  ...
  const site = await db.first('SELECT id, title FROM ms_sites WHERE id = ? AND account_id = ?', [siteId, user.id])
  if (!site) return res.redirect('/dashboard')

  try {
    await ensureTable()
    const leads = await db.query(
      'SELECT * FROM ms_biolink_leads WHERE site_id = ? ORDER BY created_at DESC',
      [siteId]
    )
