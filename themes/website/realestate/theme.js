...
$(function () {
  ...
  $(document).on('click', '.prop-wishlist', function (e) {
    ...
    $btn.data('liked', !isLiked).text(isLiked ? '♡' : '♥').css('color', isLiked ? '' : '#ef4444');
  });
  ...
  $sections.css({ opacity: 0, transform: 'translateY(18px)', transition: 'opacity .45s ease, transform .45s ease' });

  function revealOnScroll() {
    var scrollBottom = $(window).scrollTop() + $(window).height();
    $sections.each(function () {
      if ($(this).offset().top < scrollBottom - 50) {
