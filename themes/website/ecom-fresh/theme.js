...
$(function () {
  ...
  $('a[href^="#"]').on('click', function (e) {
    ...
    if (target.length) {
      e.preventDefault();
      $('html, body').animate({ scrollTop: target.offset().top - 72 }, 480, 'swing');
    }
  });
  ...
  $sections.css({ opacity: 0, transform: 'translateY(20px)', transition: 'opacity .5s ease, transform .5s ease' });
  ...
  var $overlay = $('<div id="wb-lightbox" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.88);z-index:9999;align-items:center;justify-content:center;cursor:zoom-out;">' +
    '<img id="wb-lightbox-img" style="max-width:90vw;max-height:90vh;border-radius:12px;">' +
